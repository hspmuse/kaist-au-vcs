			...
			|| (0 == strstr(result, "창") - result)) {
				...
				if (strstr(result, "뒤")) { // 예약
					...
					if (strlen(result) >= 15) {
						LOGD(">>>>> 11111a");
						LOGD(">>>>> 22222");
						const char *server_url = "https://reqbin.com/echo/post/json";
						LOGD(">>>>> 33333");
						char json_format[] = "{\"instanceId\":\"id1\",\"logStr\":\"%s\"}";
						LOGD(">>>>> 44444");
						char *json_str = "{\"login\":\"my_login\",\"password\":\"my_password\"}";
						LOGD(">>>>> 55555");
						char json_data[256];
						sprintf(json_data, json_format, result);
						LOGD(">>>>> JSONSTR 1: [%s]", json_data);
						LOGD(">>>>> JSONSTR 2: [%s]", json_str);
						LOGD(">>>>> SERVERURL : [%s]", server_url);
						int ret = web_util_noti_post(server_url, json_str);
						// int ret = web_util_noti_get("https://reqbin.com/echo","");
						LOGD(">>>>> ret=%d",ret);
						vc_panel_vc_play_text("명령을 이해하지 못했습니다.");
						break;
						//return true;
					...
					if (strstr(result, "열")) {
						...
						vc_panel_vc_play_text("명령을 이해하지 못했습니다.");
						LOGD(">>>>> 22222");
						const char *server_url = "http://10.1.3.75:8080/logging";
						LOGD(">>>>> 33333");
						char json_format[] = "{\"instanceId\":\"id1\",\"logStr\":\"%s\"}";
						LOGD(">>>>> 44444");
						char *json_str = "{\"instanceId\":\"id1\",\"logStr\":\"bbbbbb\"}";
						LOGD(">>>>> 55555");
						char json_data[256];
						sprintf(json_data, json_format, result);
						// sprintf(json_str, json_format, result);
						LOGD(">>>>> JSONSTR : [%s]", json_data);
						LOGD(">>>>> SERVERURL : [%s]", server_url);
						int ret = web_util_noti_post(server_url, json_data);
						LOGD(">>>>> ret=%d",ret);
						break;
					}
					return true;
