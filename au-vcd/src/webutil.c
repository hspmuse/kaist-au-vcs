...
{        
	struct timeval val;
        ...
        snprintf(res_time, sizeof(res_time), "%04d-%02d-%02d_%02d:%02d:%02d"
...
int web_util_noti_post(const char *resource, const char *json_data)
{
    ...
    LOGD(">>>>> web_util_noti_post1 = [%s]", resource);
	LOGD(">>>>> web_util_noti_post2 = [%s]", json_data);
	...
	if (!curl) {
		_E("fail to init curl");
		LOGD(">>>>> web_util_noti_post fail = [%s]", resource);
		return -1;
	}

	LOGD(">>>>> web_util_noti_post 4=");
	...
	LOGD(">>>>> web_util_noti_post 5=");

	response = curl_easy_perform(curl);

	LOGD(">>>>> web_util_noti_post 6=");
	// LOGD(">>>>> web_util_noti_post 7=[%s]", response);
	...
	if (response != CURLE_OK) {
		...
		LOGD(">>>>> web_util_noti_post error=");
		ret = -1;
	}

	curl_slist_free_all(headers);
	curl_easy_cleanup(curl);
	LOGD(">>>>> web_util_noti_post 10=");
	return ret;
}
